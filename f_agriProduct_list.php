...
if (isset($_GET['search_id']) && $_GET['search_id'] !== '') {
    ...
    // Fetch the product details with all its varieties
    $query = "SELECT p.product_id, p.name, p.seasonality, p.type, 
              GROUP_CONCAT(v.variety SEPARATOR ', ') AS varieties
              FROM agri_product p
              LEFT JOIN agri_product_variety v ON p.product_id = v.product_id
              WHERE p.product_id = '$search_id'
              GROUP BY p.product_id";
} else {
    // Fetch all products with their varieties
    $query = "SELECT p.product_id, p.name, p.seasonality, p.type, 
              GROUP_CONCAT(v.variety SEPARATOR ', ') AS varieties
              FROM agri_product p
              LEFT JOIN agri_product_variety v ON p.product_id = v.product_id
...
<body>
  <div>
      ...
      <tbody>
        <?php foreach ($products as $product): ?>
          <tr>
            ...
            <td><?php echo htmlspecialchars($product['varieties']); ?></td>
            <td>
    ...
    <a href="farmer_dashboard.php"><button class="btn btn-back">Back</button></a>
  </div>
</body>
</html>
